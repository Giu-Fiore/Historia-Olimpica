...
Posteriormente, se realizó una nueva columna para identificar en binario el sexo y se coloco en orden Bronze:1, Plata:2 y Oro:3 con el fin de poder realizar correctamente el análisis.
...
nmedallasxNOC<-athlete_events1%>%
  group_by(NOC)%>%
  summarise(total=sum(Sex_bin))
```

```{r}
nmedallasxNOC <- nmedallasxNOC [order(nmedallasxNOC$total, decreasing = TRUE), ]
head(nmedallasxNOC)
```

### Top 10 países con medallas de oro, plata y bronce

Se gráfican los países con mayor cantidad de medallas para cada categoría: Gold, Silver y Bronze. Para esto, primero crearemos una data donde solo estén los participantes que obtuvieron medallas en alguna competencia. Entonces, creamos una nueva data en donde eliminamos las filas que tengan Na's en la columna Medal.

```{r}
#borramos filas con medalla NA
athlete_events_not.na <- athlete_events[!is.na(athlete_events$Medal),]
```

Luego, creamos una data para cada categoría de madallas. Así, tendremos 3 tablas en donde solo estén los participantes con medalla de oro, platay bronce, respectivamente.
